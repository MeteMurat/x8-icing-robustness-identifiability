...
def main():
    ...
    files={"clean":validation_files(a.clean_dir),"iced":validation_files(a.iced_dir)}
    exact=sorted(set(files["clean"]).intersection(files["iced"]))

    if len(exact)!=10:
        raise RuntimeError("Expected 10 exact ID>=4 validation longitudinal pairs; observed %d"%len(exact))
    ...
    if not science_gate:
        ...
        classification="MODEL_SENSITIVE_VALIDATION"
    elif one_sided_p is not None and one_sided_p<=0.05:
        classification="CONFIRMED_STRONG"
    else:
        classification="DIRECTIONALLY_REPLICATED"
    ...
    # Artifacts.
    with open(os.path.join(a.out,"P1_A8D_VALIDATION_PAIR_SUPPORT.csv"),"w",newline="",encoding="utf-8-sig") as f:
        fields=[
            "file_name",
    ...
    with open(os.path.join(a.out,"P1_A8D_VALIDATION_BATTERY_POWER_BRANCH_RESULTS.csv"),"w",newline="",encoding="utf-8-sig") as f:
        fields=[
    ...
    with open(os.path.join(a.out,"P1_A8D_VALIDATION_BATTERY_POWER_COEFFICIENTS.csv"),"w",newline="",encoding="utf-8-sig") as f:
    ...
    with open(os.path.join(a.out,"P1_A8D_VALIDATION_VOLTAGE_CURRENT_DIAGNOSTIC.csv"),"w",newline="",encoding="utf-8-sig") as f:
        fields=list(vi.keys())
        w=csv.DictWriter(f,fieldnames=fields)
        w.writeheader()
        w.writerow(vi)
    ...
    audit={
        "scope":{
            ...
            "validation_battery_power_outcomes_opened":True
        },
        "support":{
            "minimum_pairs":6,
    ...
    with open(os.path.join(a.out,"P1_A8D_VALIDATION_AUDIT.json"),"w",encoding="utf-8") as f:
        json.dump(audit,f,indent=2,sort_keys=True)

    print("P1_A8D_LOCKED_VALIDATION_BATTERY_POWER_SUMMARY")
    print("exact_validation_longitudinal_pairs =",len(exact))

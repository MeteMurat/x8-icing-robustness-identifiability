...
def reconstruct(path,cfg,pms,env,half,eta_unity=False):
    ...
    for i in range(half,len(rows)-half):
        ...
        if src_cd is not None:
            src_cd_sq+=(CD-src_cd)**2
            src_cd_den+=src_cd*src_cd
            src_cd_n+=1

        src_cl=F(rows[i].get("CL"))
        if src_cl is not None:
            src_cl_sq+=(CL-src_cl)**2
            src_cl_den+=src_cl*src_cl
            src_cl_n+=1
    ...
    def nrmse(sq,den,n):
        if n and den>EPS:
            return math.sqrt(sq/n)/math.sqrt(den/n)
        return None
    ...
    return {
        ...
        "source_CD_diagnostic_nrmse":nrmse(src_cd_sq,src_cd_den,src_cd_n),
        "source_CL_diagnostic_nrmse":nrmse(src_cl_sq,src_cl_den,src_cl_n),
        "prop_thrust_reference_nrmse":nrmse(prop_sq,prop_den,prop_n),
        "rows":kept
    }
...
def build_rows(raw,ref,beta_aug=False):
    ...
    for r in raw:
        ...
        if beta_aug:
            z["beta_c"]=r["beta"]  # beta_ref frozen to 0 rad
...
def fit_response(rows,ycol,weight_mode="row",beta_aug=False):
    ...
    # Pair demeaning removes the shared pair fixed effect for slope estimation.
    ...
    # Recover shared pair fixed effects as mean residual offsets over both
    # configurations. Their equal-pair arithmetic mean is the clean reference.
    pair_intercepts={}
    for pair,rr in groups.items():
        residual_offsets=[]
        for r in rr:
            systematic=sum(beta[j]*r[cols[j]] for j in range(len(cols)))
            residual_offsets.append(r[ycol]-systematic)
        pair_intercepts[pair]=float(np.mean(residual_offsets))
...
def derived_from_two_fits(clfit,cdfit):
    CLc=clfit["clean_ref"]
    CLi=clfit["iced_ref"]
    CDc=cdfit["clean_ref"]
    CDi=cdfit["iced_ref"]
...
def summarize_loo(values):
    ...
    for key in keys:
        ...
        out[key+"_jackknife_se"]=jackknife_se(vals)
...
def fit_branch(raw,ref,weight_mode="row",beta_aug=False):
    ...
    full=derived_from_two_fits(clfit,cdfit)
    ...
    for pair in pairs:
        ...
        try:
            ...
            zz=derived_from_two_fits(clf,cdf)
...
def main():
    ...
    files={
        "clean":id_files(a.clean_dir),
        "iced":id_files(a.iced_dir)
    }

    exact=sorted(set(files["clean"]).intersection(files["iced"]))
    if len(exact)!=12:
        raise RuntimeError(
            "Expected 12 exact longitudinal identification pairs; observed %d"
            % len(exact)
        )
    ...
    # Reconstruct every required physical derivative/propulsion branch.
        ...
        for fn in exact:
            ...
            for cfg in ("clean","iced"):
                ...
                diagnostics.append({
                    ...
                    "source_CD_diagnostic_nrmse":res["source_CD_diagnostic_nrmse"],
                    "source_CL_diagnostic_nrmse":res["source_CL_diagnostic_nrmse"],
                    "prop_thrust_reference_nrmse":res["prop_thrust_reference_nrmse"]
                })

        support[bname]=bypair

    # PRIMARY_W5 defines represented pair names.
    represented=[]
    support_rows=[]
    ...
    # Required sensitivity branch row-support must remain adequate for the
    # same represented pair set. W9 is descriptive and is not a support gate.
    required_physical_branches=["PRIMARY_W5","ETA1_W5","W7"]

    sensitivity_row_support={}
    for bname in required_physical_branches:
        ok=True
    ...
    if numerical_pass:
        for name in required_fit_names:
            ...
            numerical_pass=numerical_pass and (
                ...
                z["largest_pair_fraction"]<=0.20
            )

    denominator_pass=numerical_pass
    if numerical_pass:
        denominator_pass=all(
            fits[name]["denominator_gate"]
            for name in required_fit_names
        )
    ...
    if numerical_pass and denominator_pass:
        ...
        if primary["relative_efficiency_degradation"]<=0.0:
            classification="NO_REFERENCE_EFFICIENCY_DEGRADATION"
        elif not all_required_positive:
            classification="EFFICIENCY_SIGNAL_MODEL_OR_PROPULSION_SENSITIVE"
        else:
            classification="EFFICIENCY_SIGNAL_ROBUST_IDENTIFICATION"

    elif not denominator_pass and numerical_pass:
        primary=fits.get("PRIMARY_W5")
        required_positive={}
        all_required_positive=False
        classification="INSUFFICIENT_IDENTIFICATION_SUPPORT_DENOMINATOR_GATE"
    ...
    with open(
        os.path.join(a.out,"P1_A7B_PAIR_SUPPORT.csv"),
    ...
    with open(
        os.path.join(a.out,"P1_A7B_RECONSTRUCTION_DIAGNOSTICS.csv"),
        "w",newline="",encoding="utf-8-sig"
    ) as f:
        fields=[
    ...
    with open(
        os.path.join(a.out,"P1_A7B_EFFICIENCY_BRANCH_RESULTS.csv"),
        "w",newline="",encoding="utf-8-sig"
    ) as f:
        fields=list(fit_rows[0].keys()) if fit_rows else [
            "branch","pairs","jackknife_refits",
            "CL_rank","CL_required_rank","CL_condition_number",
            "CD_rank","CD_required_rank","CD_condition_number",
            "largest_pair_fraction","denominator_gate",
            "CL_clean_ref","CL_iced_ref","delta_CL_ref",
            "delta_CL_jackknife_se","delta_CL_loo_min","delta_CL_loo_max",
            "CD_clean_ref","CD_iced_ref","delta_CD_ref","delta_CD_jackknife_se",
            "E_clean_ref","E_iced_ref","delta_E_ref","delta_E_jackknife_se",
            "relative_efficiency_degradation",
            "relative_efficiency_degradation_jackknife_se",
            "relative_efficiency_degradation_loo_min",
            "relative_efficiency_degradation_loo_max",
    ...
    coefficient_rows=[]
    for name,z in fits.items():
        for response,key in (("C_L","CL_coefficients"),("C_D","CD_coefficients")):
            for coef,val in z[key].items():
    ...
    with open(
        os.path.join(a.out,"P1_A7B_REFERENCE_MODEL_COEFFICIENTS.csv"),
        "w",newline="",encoding="utf-8-sig"
    ) as f:
        fields=["branch","response","coefficient","estimate"]
        w=csv.DictWriter(f,fieldnames=fields)
        w.writeheader()
        w.writerows(coefficient_rows)

    audit={
        "scope":{
            "identification_only":True,
            "new_A7_validation_efficiency_outcomes_opened":False,
        ...
        "absolute_lift":{
            ...
            "delta_CL_jackknife_se":None if primary is None else primary["delta_CL_ref_jackknife_se"],
    ...
    with open(
        os.path.join(a.out,"P1_A7B_AUDIT.json"),
        "w",encoding="utf-8"
    ) as f:
        json.dump(audit,f,indent=2)

    print("P1_A7B_IDENTIFICATION_ABSOLUTE_LIFT_EFFICIENCY_SUMMARY")
    print("new_A7_validation_efficiency_outcomes_opened =",False)
    ...
    if primary is not None:
        print("")
        print("PRIMARY ABSOLUTE REFERENCE COEFFICIENTS")
        ...
        print("")
        print("PRIMARY REFERENCE-STATE AERODYNAMIC EFFICIENCY")
        print("  E_clean_ref = C_L/C_D                      = {:+.9g}".format(primary["E_clean_ref"]))
        print("  E_iced_ref  = C_L/C_D                      = {:+.9g}".format(primary["E_iced_ref"]))
        print("  Delta E_ref = E_iced - E_clean             = {:+.9g}".format(primary["delta_E_ref"]))
        ...
        print("")
        print("FROZEN EFFICIENCY SENSITIVITY BRANCHES")
        for name in (
            "PRIMARY_W5",
            "ETA1_W5",
            "EQUAL_PAIR_W5",

...
def main():
    ...
    with open(os.path.join(a.out,"P1_A4A_CONSEQUENCE_METRICS.csv"),"w",newline="",encoding="utf-8-sig") as f:
        fields=list(consequences[0].keys())
        w=csv.DictWriter(f,fieldnames=fields);w.writeheader();w.writerows(consequences)

    with open(os.path.join(a.out,"P1_A4A_EXCLUDED_EFFECTS.csv"),"w",newline="",encoding="utf-8-sig") as f:
    ...
    with open(os.path.join(a.out,"P1_A4A_AUDIT.json"),"w",encoding="utf-8") as f:
